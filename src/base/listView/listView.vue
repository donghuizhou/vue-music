<template>
  <scroll class="listview" :data="data" ref="listview" :listen-scroll="listenScroll" :probe-type="probeType" @scroll="scroll">
    ...
    <div class="list-shortcut" @touchstart.stop.prevent="onShoutcutTouchStart" @touchmove.stop.prevent="onShoutcutTouchMove">
      <ul>
        <li v-for="(item, index) in shortcutList" class="item" :key="item.title" :class="{'current': currentIndex === index}" :data-index="index">{{item}}</li>
      </ul>
    </div>
